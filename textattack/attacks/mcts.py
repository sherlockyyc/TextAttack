...
class MCTS(Attack):
    ...
    def update_gRAVE(self, gRAVE, F, reward_V ):
        # update gRAVE score for each feature of feature subset F, by adding the reward_V the the score
        for fi in range(gRAVE[0].shape[0]):
            if F[fi]:
                gRAVE[0][fi] = (gRAVE[0][fi]*gRAVE[1][fi] + reward_V)/(gRAVE[1][fi] + 1)
                gRAVE[1][fi] += 1 


    def UCB(self, node, gRAVE, params):
    # b<1
        ...
        if (pow((node.T_f+1),params.b)-pow(node.T_f,params.b)>1):
            if not node.allowed_features:
                return -1
            else:
                for ft in range(f):
                    if not ft in allowed_features:
                        beta = params.cl/(params.cl + node.lrave_count[ft])
                        rst = (1-beta) * lrave_score[ft] + beta * gRAVE[0][ft]
                        if rst>nowbest:
                            ft_now = ft
                            nowbest = rst
    ...
    def update_Tree_And_Get_Address(self, tree, current_features, params):
        if not tree.find(current_features):
            node = Node(current_features)
            tree.save(current_features, node)
        else:
            node = tree.find(current_features)
        return node

    def iterate(self, tree, gRAVE, current_features, params, depth):
        if depth>=params.max_depth:
            reward_V = params.get_reward(current_features, params)
            self.update_gRAVE(gRAVE, current_features, reward_V)
        else:
            next_node = self.update_Tree_And_Get_Address(tree, current_features, params)

            if (next_node.T_f != 0):
                fi = self.UCB(next_node, gRAVE, params)
                if (fi==-1): # it means that no feature has been selected and that we are going to perform random exploration
                    ...
                    reward_V = self.iterate_random(tree, current_features, params)
                    self.update_gRAVE(gRAVE, current_features, reward_V)
                else: #add the feature to the feature set
                    current_features[fi] = True
                    reward_V = self.iterate(tree,gRAVE,current_features,params,depth+1)
            else:
    ...
    def iterate_random(self, tree, current_features, params):
        f_num = current_features.shape[0]
        f_size  = current_features.sum()
        while (f_size < params.max_depth):
            if (f_num<=f_size):
                break
        ...
        return params.get_reward(self, current_features, params)

    def reward(self, f, params):
        #global bst
        score = 0
    ...
    def runmcts(self):
        for i in range(self.lcount):
            ...
            self.iterate(self.tree, self.gRAVE, current_features, self.params, 0)  

    
if __name__  == '__main__':
    m_attk = MCTS(None, MCTS.reward, 10, 100000, 100)
    m_attk.runmcts()
    print(m_attk.gRAVE)